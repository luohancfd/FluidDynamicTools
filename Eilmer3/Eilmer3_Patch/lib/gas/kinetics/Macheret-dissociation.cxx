  ...
  : Generalised_Arrhenius(L, g, T_upper, T_lower)
{
  ...
  if ( X->get_type()=="monatomic" )
    monatomic_collider_ = true;
  else
    monatomic_collider_ = false;
  double M_c = X->get_M();
...
Macheret_dissociation::
Macheret_dissociation(double A, double n, double E_a, double T_upper, double T_lower, string v_name, string c_name, double khigh)
