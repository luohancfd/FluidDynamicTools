...
contains
  subroutine EXPCOL_INIT()
    ...
    !N2-N2  SSE: 6.305792e-04
    EXPCOLPair(1,1) = 1
    INONVHS(1,1) = 2
    EXPpar(1)%sp1 = 1; EXPpar(1)%sp2 = 1
    EXPpar(1)%alpha = 3.160000d+00
    EXPpar(1)%A = 2.290000d+03
    EXPpar(1)%totxsecpar = (/1.472924d+01, 1.052763d+00, 6.106255d-02/)

    !N2-N  SSE: 7.370794e-04
    ...
    EXPpar(2)%totxsecpar = (/1.329189d+01, 1.083702d+00, 6.483460d-02/)

    !N2-O2  SSE: 7.987184e-04
    ...
    EXPpar(3)%totxsecpar = (/1.491124d+01, 1.050124d+00, 6.324566d-02/)

    !N2-O  SSE: 1.485905e-04
    ...
    EXPpar(4)%totxsecpar = (/1.014653d+01, 1.065933d+00, 5.436974d-02/)

    !N2-NO  SSE: 3.835454e-04
    ...
    EXPpar(5)%totxsecpar = (/1.359009d+01, 1.048193d+00, 5.775123d-02/)

    !N-O2  SSE: 2.933599e-04
    ...
    EXPpar(6)%totxsecpar = (/1.187605d+01, 1.072511d+00, 5.776633d-02/)

    !N-O  SSE: 7.694945e-04
    ...
    EXPpar(7)%totxsecpar = (/1.260779d+01, 1.101877d+00, 6.645368d-02/)

    !N-NO  SSE: 2.594235e-04
    ...
    EXPpar(8)%totxsecpar = (/1.187821d+01, 1.072350d+00, 5.659715d-02/)

    !O2-O2  SSE: 1.071328e-03
    EXPCOLPair(3,3) = 9
    INONVHS(3,3) = 2
    EXPpar(9)%sp1 = 3; EXPpar(9)%sp2 = 3
    EXPpar(9)%alpha = 2.850000d+00
    EXPpar(9)%A = 8.200000d+02
    EXPpar(9)%totxsecpar = (/1.517986d+01, 1.047217d+00, 6.601000d-02/)

    !O2-O  SSE: 1.714903e-04
    ...
    EXPpar(10)%totxsecpar = (/1.062548d+01, 1.063752d+00, 5.483228d-02/)

    !O2-NO  SSE: 3.320000e-04
    ...
    EXPpar(11)%totxsecpar = (/1.313519d+01, 1.043691d+00, 5.702576d-02/)

    !O-NO  SSE: 3.440941e-04
    ...
    EXPpar(12)%totxsecpar = (/1.221911d+01, 1.069347d+00, 5.875725d-02/)

    !NO-NO  SSE: 6.130441e-04
    EXPCOLPair(5,5) = 13
    INONVHS(5,5) = 2
    EXPpar(13)%sp1 = 5; EXPpar(13)%sp2 = 5
    EXPpar(13)%alpha = 3.260000d+00
    EXPpar(13)%A = 2.160000d+03
    EXPpar(13)%totxsecpar = (/1.416728d+01, 1.048699d+00, 6.156690d-02/)
  ...
  subroutine EXPCOL_Bmax(LS, MS, ET, Bmax, IERROR)
    ...
    if (iexpair .eq. 0) then
      ...
      Bmax = p(1)*dexp(-p(2)*ET**p(3))
    end if
  end subroutine EXPCOL_Bmax
  ...
  function EXPCOL_DENOMS(r)
    ...
    EXPCOL_DENOMS = 1.0d0 - temp_b**2/r**2 - EXPCOL_POT(temp_ipair, r)/temp_et
