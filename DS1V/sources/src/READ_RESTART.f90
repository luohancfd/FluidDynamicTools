...
!
101 CONTINUE
OPEN (7,FILE='PARAMETERS.DAT',FORM='UNFORMATTED',ERR=101,ACTION='READ') !--isebasti: replace binary by unformatted
...
ALLOCATE (COLLS(NCELLS),WCOLLS(NCELLS),CLSEP(NCELLS),REAC(MNRE),SREAC(MNSR),VAR(23,NCELLS), &
          VARSP(0:13,NCELLS,MSP),VARS(0:32+MSP,2),CS(0:8+MMVM,NCELLS,MSP),CSH(5,NCELLS,MSP),CSS(0:8,2,MSP,2), &  !--isebasti: correcting CS allocation
...
!
102 CONTINUE
OPEN (7,FILE='RESTART.DAT',FORM='UNFORMATTED',ERR=102,ACTION='READ') !--isebasti: replace binary by unformatted
...
IF (MNRE >0 .AND. IMF .ne. 0) THEN
  READ(7) NMFANG, MFRMASS, IMFpair, ZCHECK
  IF (ZCHECK /= 1234567) THEN
    WRITE (9,*) 'Wrong MF model information, Check integer =',ZCHECK
    STOP
  END IF
  IF (IMFS == 1 .AND. NMFpair > 0)THEN
    INQUIRE(FILE='RESTART_MF_MODEL.DAT', EXIST=FEXIST)
    IF (FEXIST) THEN
      OPEN (11,FILE='RESTART_MF_MODEL.DAT',FORM='UNFORMATTED',ACTION='READ')
      READ(11) NMFER0,NMFET0,NMFEV0,NMFER,NMFET,NMFEV,&
      ...
      END IF
      CLOSE(11)
    ELSE IF (MNRE > 0  .and. IMF .ne. 0 .and. IREAC == 2 .and. IMFS == 1) THEN
      CALL MF_INIT_SAMPLE()
    END IF
  END IF
  IF (IMF >= 2) THEN
...
READ(7) INONVHS, IQCTVT, ZCHECK
IF (ZCHECK /= 1234567) THEN
  WRITE (9,*) 'Wrong ME-QCT-VT model information, Check integer =',ZCHECK
  STOP
END IF
