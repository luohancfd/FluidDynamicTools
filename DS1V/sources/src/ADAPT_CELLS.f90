...
!
DEALLOCATE (JDIV,CELL,ICELL,CCELL,ICCELL,COLLS,WCOLLS,CLSEP,VAR,VARSP,CS,CSH,CST,STAT=ERROR) !--isebasti: CST included
IF (ERROR /= 0) THEN
...
ALLOCATE (COLLS(NCELLS),WCOLLS(NCELLS),CLSEP(NCELLS),VAR(21,NCELLS),VARSP(0:11,NCELLS,MSP),&
          CS(0:8+MMVM,NCELLS,MSP),CSH(5,NCELLS,MSP),CST(0:4,NCELLS),STAT=ERROR)  !--isebasti: correcting CS allocation
IF (ERROR /= 0) THEN
