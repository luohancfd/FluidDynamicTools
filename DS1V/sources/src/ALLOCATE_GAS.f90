...
IF (MNRE > 0 .AND. IMF .ne. 0) THEN
  ALLOCATE(NMFANG(MNRE),MFRMASS(3,MNRE),IMFpair(MSP,MSP),STAT = ERROR)
...
!-- Allocate variables related to nonVHS model and QCT-VT model
ALLOCATE(INONVHS(MSP,MSP), IQCTVT(MSP,MSP), STAT=ERROR)
IF (ERROR /= 0) THEN
