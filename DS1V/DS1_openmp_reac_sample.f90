...
REAL(8),DIMENSION(0:100,1000) :: NMFVT0,NMFVT,NMFVTR
...
IMPLICIT NONE
!
INTEGER :: NSAMP,NMISAMP,NOUT,NDISSOC,NRECOMB,NBINS,NSNAP,NSPDF,NSCELLS,IENERS  !--isebasti: included NBINS,NSNAP,NSPDF
...
IMPLICIT NONE
!
INTEGER :: IRUN,NSEED,N,I,J,IRETREM,ISET
...
IPRS=0       !post-reaction vibrational levels are populated using 0 LB, 1 pre-reaction pdf sampling, 2 modified LB
...
IF (ERROR /= 0) THEN
  WRITE (*,*) 'PROGRAM COULD NOT ALLOCATE SPACE FOR MF related ARRAYS',ERROR
ENDIF
!
CALL ALLOCATE_GAS
!
...
!
  ...
  OPEN(3,FILE='REAC_COUNT.DAT',ACCESS = 'APPEND')
  WRITE(3,*) '#Number of reaction',char(9),'Evrem (K)',char(9),'Evrem/D',char(9),'Evrem (eV)'
  IF (MINVAL(REAC) == 0.0d0) THEN
    WRITE(3,'(10(G13.6,3x))') (REAC(I),I=1,MNRE),(REAC(I),I=1,MNRE),&
        (REAC(I),I=1,MNRE),(REAC(I),I=1,MNRE) !just write zero
  ELSE
    WRITE(3,'(10(G13.6,3x))') (REAC(I),I=1,MNRE),(EVREM(I)/REAC(I),I=1,MNRE),&
        (EVREM(I)*BOLTZ/REAC(I)/REA(2,I),I=1,MNRE),(EVREM(I)/REAC(I)*BOLTZ/1.60218d-19,I=1,MNRE)
  END IF
  CLOSE(3)
END IF
!
...
IF (IREAC == 2 .AND. IMF > 0 .AND. MNRE == 1) THEN
  ...
  WRITE(3,*) 'VARIABLES = "E/kT","ET0_N","ET0_P",',&
      '"ER0_N","ER0_P","ET_N","ET_P","ER_N","ER_P",',&
      '"ETR_N","ETR_P","ERR_N","ERR_P","PEQ","PCOL","PBOLTZ"'
  DO L = 1,MNRE
    WRITE(3,7788) 1000,L
    A = SUM(NMFET(:))
    C = SUM(NMFER(:))
    ...
    DO N = 1,1000
      ...
      WRITE(3,7789) B,NMFET0(N),NMFET0(N)/AA,&
                    ...
                    NMFET(N),NMFET(N)/A,&
                    NMFER(N),NMFER(N)/C, &
                    NMFETR(N),NMFETR(N)/AS, &
                    NMFERR(N),NMFERR(N)/SRR,&
                    gamain(B,1.5d0,I) - gamain(B-0.01d0,1.5d0,I),&
                    gamain(B,CC,I) - gamain(B-0.01d0,CC,I),&
                    dcosh(B-0.01d0)-dcosh(B)-dsinh(B-0.01d0)+dsinh(B)
    END DO
    WRITE(3,*)
  END DO
  ...
  WRITE(3,*)'VARIABLES = "Ev (eV)","Ev0_N","Ev0_P","Ev_N","Ev_P","EvR_N","EvR_P","P0"'
  DO L = 1,MNRE
    WRITE(3,7788) 101,L
    A = SUM(NMFEV)
    C = SUM(NMFEVR)
    CC = SUM(NMFEV0)
    DO N = 0,100
      CALL VIB_ENERGY(B,N,1,IREA(1,L))
      WRITE(3,7789) B/1.60218d-19,NMFEV0(N),NMFEV0(N)/CC,NMFEV(N),NMFEV(N)/A,&
                     NMFEVR(N), NMFEVR(N)/C, DEXP(-B/BOLTZ/FVTMP0)
    END DO
    WRITE(3,*)
  END DO
  CLOSE(3)
  ...
  WRITE(3,*)'VARIABLES = "Et/kT", "Ev (eV)","N","NR1","NR2","P1","P2"'
  DO L = 1,MNRE
    WRITE(3,7790) 1000,101,L
    DO N = 0,100
      DO K = 1,1000
        CALL VIB_ENERGY(C,N,1,IREA(1,L))
        A = 0.0D0
        B = 0.0D0
        IF (NMFVT0(N,K) .NE. 0.0d0)  A = NMFVT(N,K)/NMFVT0(N,K)
        IF (NMFVT(N,K) .NE. 0.0d0)   B = NMFVTR(N,K) / NMFVT(N,K)

        WRITE(3,7789) DFLOAT(K)*0.01d0, C/1.60218d-19,NMFVT0(N,K),NMFVT(N,K),NMFVTR(N,K),&
          A,B
      END DO
    END DO
    WRITE(3,*)
...
!--sample pre-reaction vibrational levels
  DO KV=1,MMVM
    ...
    IF (IREAC == 2 .and. NPM == 3 .and. IMF > 0 .and. KV == 1) THEN
      ...
      ELSE
        II = FLOOR(ECR2/BOLTZ/FTMP0/0.01D0)+1
      END IF
      II = MIN(II,1000)
      NMFERR(II) = NMFERR(II)+1.0d0

      II = FLOOR(ECT/BOLTZ/FTMP0/0.01D0)+1
      II = MIN(II,1000)
      NMFETR(II) = NMFETR(II)+1.0d0
      NMFEVR(I) = NMFEVR(I) + 1.0d0
      NMFVTR(I,II) = NMFVTR(I,II) + 1.0d0
    ENDIF
    !$omp end critical
  END DO
!
!--sample pre-reaction vibrational energies (normalized by 1000*BOLTZ)
  ...
  !$omp critical
  IF (IVDC == 1) THEN
    EVREM(IKA) = EVREM(IKA) + ECV1/BOLTZ
  ELSE
    EVREM(IKA) = EVREM(IKA) + ECV2/BOLTZ
  END IF
  !$omp end critical 
!
!   -----------------------------------------------------------
...
!--Calculate the VT (ME-QCT) cross-sections (only for O2+O and N2+O collisions)
            ...
            IF ((IREAC .eq. 2) .and. (IMF .gt. 0) .and. (MNRE == 1)) THEN
              ...
              NMFET0(IETDX) = NMFET0(IETDX) + 1.0D0
              ...
              IF (IEVDX >= 0 )THEN
                NMFEV0(IEVDX) = NMFEV0(IEVDX) + 1.0d0
                NMFVT0(IEVDX,IETDX) = NMFVT0(IEVDX,IETDX)+1.0D0
              END IF
            END IF
!
            VTXSECTION=0.d0
...
!
              ...
              VTXSECTION(:)=SIGMA_REF*ET**(QNU-1.d0)*VTXSECTION(:) !SIGMA_ME-QCT-VT in Angstrons^2
            END IF
!
...
!-------------------------------------------------
              IF ((IREAC .eq. 2) .and. (IMF .gt. 0) .and. (MNRE == 1)) THEN
                ...
                IF (IEVDX >= 0 )THEN
                  NMFEV(IEVDX) = NMFEV(IEVDX) + 1.0d0
                  NMFVT(IEVDX,IETDX) = NMFVT(IEVDX,IETDX)+1.0D0
                END IF
              END IF
!
...
!-------------------------------------------------
                ...
                IF ((IKA /= 0).AND.(IPRS == 2)) THEN
                      ...
                      IF (JX == 3) THEN
                        ...
                        END IF
                      END IF
                      BB=5.d0-2.d0*SPM(3,LS,MS)   !translational dofs
                      BB=BB+ISPR(1,LS)+ISPR(1,MS) !rotational dofs
                      BB=BB+SUM(SVDOF)            !vibrational dofs
                END IF
!
                DO NSP=IAX,KK,IVM
                      IF (IVDC < 2) THEN
                                ...
                                DO WHILE ((II == 0).and.(nstep < 100000))
                                  ...
                                  IF (EVIB < ECC) THEN
                                    ...
                                    IF ((IKA /= 0).AND.(IPRS == 2)) THEN  !modified LB for reacting collisions
                                      ...
                                      BB=BB-B
                                      A=BB    !remaining degrees of freedom
                                    END IF
