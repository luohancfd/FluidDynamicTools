...
def read_elements(sif, num_points, num_elements, ug, three_d_file):
  ...
  for line in sif:
    ...
    elif s and len(s.split()) == num_items_per_line+1 :
      if three_d_file:
        tri = vtk.vtkTriangle
        etype = int(s.split()[1]) 
        i = int(s.split()[2])
...
if __name__ == "__main__":
  ...
  parser.add_argument('-t', '--tecplot', action='store_true', 
                      default=False, help="Create TecPlot ASCII plt files intead of VTK vtu")
  ...
  group.add_argument('--type', help="Type for optional list of SPARTA dump result files", nargs='+')
  ...
  if os.path.isdir(args.paraview_output_file):
    print "WARNING: ParaView output directory exists: ", args.paraview_output_file
    #sys.exit(1)
  ...
  if not os.path.isdir(args.paraview_output_file):
  ...
  for time in sorted(time_steps_dict.keys()):
    ...
    if args.tecplot:
      fh = open(os.path.join(args.paraview_output_file, args.paraview_output_file + '_' + str(time) + '.plt'), "w")
      vtk2tecplot.write_tecplot_ascii(ug, fh, args.paraview_output_file + " at time = " + str(time), time) 
      fh.close()
    else:
